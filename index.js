...
function searchForDuplicateBooks(bookToSearch) {
  if (!(bookToSearch instanceof Book)) throw new Error('on searchForDuplicateBooks(). Provided argument is not a Book');
...
function addBookToLibrary(bookToAdd) {
  if (!(bookToAdd instanceof Book)) throw new Error('on addBookToLibrary(). Provided argument is not a Book');

  bookLibrary.push(bookToAdd);
}

function deleteBookFromLibrary(bookToDelete) {
  if (!(bookToDelete instanceof Book)) throw new Error('on deleteBookFromLibrary(). Provided argument is not a Book');
...
function displayAddBookDialog() {
  ...
  addBookToLibraryButton.addEventListener('click', (event) => {
    ...
    // When we retrieve the value from the form,
